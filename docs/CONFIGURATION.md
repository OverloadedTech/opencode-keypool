...
`~/.config/opencode-keypool/config.json` (or `--config <path>`).
`$XDG_CONFIG_HOME` is respected. The file is created by `oc-keypool tui`
...
| `exhaust_wait_timeout_seconds` | `3600` | Total time a request may hold while *all* entries are cooling before it fails with `502 AllEntriesFailed`. `0` = wait forever. |
| `pools` | `[]` | Ordered pools, one model per pool is exposed to opencode as `keypool/<pool id>`. |

## Pools

```jsonc
{
  "id": "default",     // exposed to opencode as keypool/default
...
| `label` | string | Shown in the TUI and in the `x-keypool-entry` response header. |
...
The keypool countermeasure is **per-entry `proxy`**: give each entry its
own egress IP (a different VPN, VPS, or proxy per account), and each key
gets its own limit bucket. Example with two accounts and two IPs:
...
| Variable | Effect |
|---|---|
| `XDG_CONFIG_HOME` | Base for `opencode-keypool/config.json` (default `~/.config`). |
